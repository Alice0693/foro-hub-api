...
Filtro de Seguridad: Un filtro personalizado (SecurityFilter) intercepta cada solicitud, valida el token, extrae el usuario y autoriza el acceso al recurso solicitado,
manteniendo la arquitectura Stateless (sin estado).

Gemini ha dicho
¡Excelente iniciativa! Un README.md detallado no solo explica qué hace tu código, sino cómo lo protege. Esto demuestra que comprendes la importancia de la seguridad en aplicaciones web, algo muy valorado en entornos empresariales.

Aquí tienes una versión extendida y profesional que puedes copiar en tu README.md. He incluido una sección técnica dedicada a la autenticación JWT para que el reclutador vea tu dominio del tema.
...
Testing de API: Insomnia / Postman
